...
│   ├── index.md   # 전체 목차
│   ├── log.md     # 변경 이력
│   ├── concepts/  # 개념 엔티티 (LLM Wiki, 세컨드브레인 등)
│   ├── entities/  # 회사/인물/제품/도구
│   ├── sources/   # 소스별 요약
│   └── synthesis/ # 통합 분석
├── output/        # 산출물 (스크립트, 보고서)
└── graphify-out/  # 지식 그래프 (graph.json, graph.html)
```
...
| 작업 | 복잡한 인제스트, 스킬 생성, 시스템 설계 | 빠른 수집, 간단 인제스트, 빠른 쿼리, 크론 자동화 |
| 접근 | 터미널, IDE | 디스코드, CLI |
...
raw/ 파일 → 개념/엔티티 추출 → wiki/concepts/ 또는 wiki/entities/
        ...
        → wiki/index.md 업데이트
        → wiki/log.md 기록
        → frontmatter에 status: processed 추가
```
...
**참고**: 자동 분류는 하지 않음. 사용자가 직접 분류하거나 raw/에 파일을 둘 때 적절한 폴더를 선택
